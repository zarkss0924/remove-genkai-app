...
if [ $? -eq 0 ]; then
  adb shell pm list package | sed -e "s/package://g" | grep -e 'sprd' -e 'ukelink' -e 'spreadtrum' -e 'spread' -e 'ucloudlink' -e 'softbank' | sed "s@^@adb shell pm uninstall --user 0 @g" > test.sh
